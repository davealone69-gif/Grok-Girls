...
export const HDRenderView = forwardRef<HDRenderViewHandle, HDRenderViewProps>(function HDRenderView(props, ref) {
  ...
  const definition = () => ({
    ...
    skin: props.skin ?? 'Skin 1',
    hair: props.hair ?? 'Long',
    eyes: props.eyes ?? 'Natural',
    face: props.face ?? 'Natural',
    body: props.body ?? 'Average',
    outfit: props.outfit ?? 'Tech',
    age: props.age ?? 'Adult',
    tattoos: props.tattoos ?? 'None',
    augmentations: props.augmentations ?? 'None'
  });
  ...
  const renderNow = () => {
    ...
    renderer.configure({
      resolution: props.resolution ?? RenderResolution.FULL_HD,
  ...
  useEffect(() => {
    ...
    // Render configuration is intentionally captured at mount; parent can call render().
