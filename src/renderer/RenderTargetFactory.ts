...
function colorFallback(requested: ColorInternal, caps: RenderTargetCapabilities): ColorInternal {
  const order: ColorInternal[] = requested === 'rgba8'
    ? ['rgba8']
    : requested === 'r16f'
      ? ['r16f', 'rgba8']
      : requested === 'rg16f'
        ? ['rg16f', 'rgba16f', 'rgba32f', 'rgba8']
        : requested === 'rgba32f'
...
function depthFallback(requested: DepthInternal, caps: RenderTargetCapabilities): DepthInternal {
  ...
  const order: Exclude<DepthInternal, null>[] = requested === 'depth16'
    ? ['depth16']
    : requested === 'depth24'
      ? ['depth24', 'depth16']
...
export function createRenderTarget(gl: WebGL2RenderingContext, request: RenderTargetRequest): RenderTarget {
  ...
  if (depthFormat) {
    ...
    if (!depthBuffer) {
      ...
      throw new Error('Unable to allocate depth buffer');
    }
  ...
  return {
    ...
    colorTexture,
    depthBuffer,
    colorInternal: color.internal,
    colorFormat,
    depthInternal: depthFormat ? depthDescriptor(gl, depthFormat) : null,
    depthFormat,
    width: request.width,
