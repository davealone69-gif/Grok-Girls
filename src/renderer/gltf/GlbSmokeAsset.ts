...
/** Minimal valid GLB containing one textured-ready triangle. Useful for CI parser/GPU smoke tests. */
export function createGlbSmokeAsset(): ArrayBuffer {
  ...
  for (const chunk of chunks) { offsets.push(cursor); cursor += chunk.byteLength; }
  const bin = new Uint8Array(cursor);
  chunks.forEach((chunk, i) => bin.set(chunk, offsets[i]));
  const json = jsonChunk({
    asset: { version: '2.0', generator: 'Grok-Girls HD CI' },
    scene: 0,
    scenes: [{ nodes: [0] }],
    nodes: [{ mesh: 0 }],
