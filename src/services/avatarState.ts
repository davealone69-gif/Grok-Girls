...
export function saveAvatarState(id:string,state:AvatarState){try{localStorage.setItem(`${KEY}:${id}`,JSON.stringify(state))}catch(e){console.warn('[avatar] could not persist state (storage full?)',e)}}
