...
export function buildGenerationPrompt(girl:Girl,room:Room,prompt:string,mode:Mode,enhanced:boolean,interaction?:string){const memories=contextMemories(girl,room.id).map(m=>m.summary).join('; ');return `${buildAvatarPrompt(girl,room,interaction,mode,enhanced)} Relationship: ${relationshipModifier(girl)}. ${memories?`Relevant memory: ${memories}.`:''} User direction: ${prompt.trim()||'Create a polished showcase scene'}`}
